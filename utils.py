...
@dataclass
class DataCollatorSpeechSeq2SeqWithPadding_from_npy:
    processor: Any

    def __call__(self, features: List) -> Dict[str, torch.Tensor]:
        # split inputs and labels since they have to be of different lengths and need different padding methods
        # first treat the audio inputs by simply returning torch tensors
        input_features = [{"input_features": np.load(feature["input_features"])} for feature in features]
        batch = self.processor.feature_extractor.pad(input_features, return_tensors="pt")
...
def compute_metrics(pred, wer_metric, cer_metric, tokenizer):
...
def prepare_dataset_aihub(batch, tokenizer, feature_extractor, sr=16000):
